...
#              print "ikj_dist: %s"%ikj_dist + "k: %s"%k
#	      print "ij0: %s"%path_map[i][j][0] + "ij1: %s"%path_map[i][j][1]
...
#  dump()
#  dump2()
  for k in reversed(sws):
    for i in reversed(sws):
      for j in reversed(sws):
        if path_map[i][k][0] is not None:
          if path_map[k][j][0] is not None:
            # i -> k -> j exists
            ikj_dist = path_map[i][k][0]+path_map[k][j][0]
	...
	def run(self):
		try:
			while self.keeprunning:
				...
				global get_Stat
				get_Stat = True
				response = urllib2.urlopen('http://10.0.2.15:8080/stats/1/1/0/0')
				html = response.read()
				if html == "[\" \"]":
					time.sleep(3)
					response = urllib2.urlopen('http://10.0.2.15:8080/stats/1/1/0/0')
					html = response.read()
				stats_list = html.split("], [")
				latency =[]
				for item in stats_list: 
					r=item.split(",")
					latency.append(r[3])
				del latency[len(latency)-1]
				sum = 0
				for item in latency:					
 					sum += float(item)
				avg = sum/len(latency)
